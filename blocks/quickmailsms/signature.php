...
$header = quickmailsms::_s('signature');
...
$PAGE->set_url('/blocks/quickmailsms/signature.php', array(
...
$dbsigs = $DB->get_records('block_quickmailsms_sigs', $params);
...
$sig = file_prepare_standard_editor($sig, 'signature', $options, $context,
    'block_quickmailsms', 'signature', $sig->id);

$form = new signature_form(null, array('signature_options' => $options));

if ($confirm) {
    $DB->delete_records('block_quickmailsms_sigs', array('id' => $sigid));
    redirect(new moodle_url('/blocks/quickmailsms/signature.php', array(
        'courseid' => $courseid,
        'flash' => 1
    )));
}
...
if ($form->is_cancelled()) {
    ...
    if (empty($data->title)) {
        $warnings[] = quickmailsms::_s('required');
    }
    ...
    if (empty($warnings) and empty($delete)) {
        ...
        $default = $DB->get_record('block_quickmailsms_sigs', $params);

        if ($default and !empty($data->default_flag)) {
            $default->default_flag = 0;
            $DB->update_record('block_quickmailsms_sigs', $default);
        }
        ...
        if (empty($data->id)) {
            $data->id = null;
            $data->id = $DB->insert_record('block_quickmailsms_sigs', $data);
        }

        // Persist relative links
        $data = file_postupdate_standard_editor($data, 'signature', $options,
            $context, 'block_quickmailsms', 'signature', $data->id);

        $DB->update_record('block_quickmailsms_sigs', $data);
...
$first = array(0 => 'New '.quickmailsms::_s('sig'));
...
if (!empty($delete)) {
    $msg = get_string('are_you_sure', 'block_quickmailsms', $sig);
    $confirm_url = new moodle_url('/blocks/quickmailsms/signature.php', array(
        'id' => $sig->id,
        'courseid' => $courseid,
        'confirm' => 1
    ));
    $cancel_url = new moodle_url('/blocks/quickmailsms/signature.php', array(
        'id' => $sig->id,
        'courseid' => $courseid
    ));
