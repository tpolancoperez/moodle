<?php // $Id: checkseats.php,v 1.1.2.3 2009/10/22 14:28:23 jfilip Exp $

/**
 * Checks to see if the seats requested are available over a given period of time.
 *
 * @version $Id: checkseats.php,v 1.1.2.3 2009/10/22 14:28:23 jfilip Exp $
 ...
 */
    ...
    if (!empty($cmid)) {
        if (! $cm = get_coursemodule_from_id('elluminatelive', $cmid)) {
...
/// Some capability checks.
    ...
    require_capability('mod/elluminatelive:manageseats', $context);


    $strtitle = !empty($meetingname) ?
                get_string('checkavailabilityfor', 'elluminatelive', $meetingname) :
                get_string('checkavailabilityfor', 'elluminatelive', get_string('meeting', 'elluminatelive'));

/// Print header with no info for a pop-up.
    print_header();
...
/// checking for if this is trying to adjust seat reservation for an exsting meeting).
    if (!empty($meetingid)) {
        $seats = elluminatelive_get_max_available_seats($timestart, $timeend, $meetingid);
    } else {
        $seats = elluminatelive_get_max_available_seats($timestart, $timeend);
    }
    ...
    if ($seats === false) {
        print_heading(get_string('couldnotgetavailableseatinfo', 'elluminatelive'));
    } else if ($seatcount > $seats) {
        notify(get_string('reservedseatsno', 'elluminatelive', $a), 'notifyproblem', 'left');
    } else {
        notify(get_string('reservedseatsyes', 'elluminatelive', $a), 'notifysuccess', 'left');
    }
