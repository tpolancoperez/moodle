...
class attendance_tabs implements renderable {
	...
	// COPIED VARIABLES TO THIS FILE. TP. 10/24/13
        if(isset($this->att->perm->admin) && ($this->att->perm->admin == true) ) {
		// DOC RICHARDS WANTED IT SO THAT NO ONE COULD ADD A SESSION
		// SO WE JUST PULL THE TAB ALTOGETHER
		// COPIED COMMENTS TO THIS FILE. TP. 10/24/13	
            $toprow[] = new tabobject(self::TAB_ADD,
                                     $this->att->url_sessions()->out(false, array('action' => att_sessions_page_params::ACTION_ADD)),
