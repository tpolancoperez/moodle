...
//  generates sessions
	...
	/// Print headers
        ...
        $sql = "SELECT MIN(sessdate) AS min";
            $sql.= ", MAX(sessdate) AS max";
        $sql.= " FROM {attendance_sessions}";
        $sql.= " WHERE courseid=$course->id";
        ...
        echo '<center>';
        echo $OUTPUT->help_icon('display', 'attforblock');
        echo get_string('display','attforblock').': ';
        if (isset($_GET['view']))
            set_current_view($course->id, $_GET['view']);
        $view = get_current_view($course->id);
        echo $OUTPUT->single_select("report.php?id=$id&amp;sort=$sort&amp;view=", 'view', $options, $view, '');
        ...
        if ($view === 'weeks') {
                ...
                for ($i = 0, $monday = $startdate; $monday <= $lastdate; $i++, $monday += ONE_WEEK) {
                        if ($DB->count_records_select('attendance_sessions', "courseid={$course->id} AND sessdate >= ".$monday." AND sessdate < ".($monday + ONE_WEEK))) {
                                $list[] = $monday;
                        }
                }
        } elseif ($view === 'months') {
                ...
                for ($i = 0, $month = $startdate; $month <= $lastdate; $i++, $month = make_timestamp($syear, $smonth+$i, 1)) {
                        if ($DB->count_records_select('attendance_sessions', "courseid={$course->id} AND sessdate >= ".$month." AND sessdate < ".make_timestamp($syear, $smonth+$i+1, 1))) {
                                $list[] = $month;
                        }
                }
        }
        ...
        if ($view === 'weeks') {
                ...
                $where = "courseid={$course->id} AND sessdate >= $current AND sessdate < ".make_timestamp($syear, $smonth+1, 1);
        } else {
                $where = "courseid={$course->id} AND sessdate <= ".time();
        }
        
        if ($students &&
            ($course_sess = $DB->get_records_select('attendance_sessions', $where, null, 'sessdate ASC'))) {
            ...
            foreach($students as $student) {
                $table->data[$student->id][] = $OUTPUT->user_picture($student, array("courseid"=>$course->id));
