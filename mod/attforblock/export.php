...
//  Lists all the sessions for a course
    ...
    if ($fromform = $mform_export->get_data()) {
        ...
        $students = get_users_for_attendance($context, '', $group);
...
//			$data->sheettitle .= $group ? $group->name : get_string('allparticipants');
            ...
            if (isset($fromform->includenottaken)) {
                    $select .= " AND sessdate <= :sessionenddate";
                    $params["sessionenddate"] = $fromform->sessionenddate;
            } else {
                    $select .= " AND lasttaken != 0";
            }

            if ($sessions = $DB->get_records_select('attendance_sessions', $select, $params, 'sessdate ASC')) {
                    foreach($sessions as $sess) {
                            $data->tabhead[] = userdate($sess->sessdate, get_string('strftimedmyhm', 'attforblock'));
                    }
            } else {
                notice(get_string('nosessionsforexport', 'attforblock'), 'export.php?id='.$id);
            }
            $data->tabhead[] = '%';
