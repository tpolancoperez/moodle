...
////////////////////////////////////////////////////////////////
    ...
    $PAGE->set_url('/add_attendance_process.php', $urlparams);
...
if($adminator == true)
{
// begin if $adminator enclosure
$date = date("Y_m_d");
$logfile = "Attendance_Add_Log_".$date.".txt";
$directory = "/var/moodle_www/attendance_logs/";
$fh = fopen($directory.$logfile,'w');
foreach($course_categories as $cat_id)
{
	echo "<h2>Beginning Attendance Module Additions</h2>";
...
//*/
		...
		// insert one session
		//
		$logitem = "Added MODULE 'ATTENDANCE' to $course->shortname (# $course->id)\n";
		echo $logitem."<br>";
		...
		{
			$logitem1 = "-----&gt;SUCCESS! - Attendance Session for COURSE #".$course->id." and Instance # $rec->attendanceid Inserted\n";
			echo $logitem1."<br>";
		}
		else
		{
			$logitem1 = "-----&gt;FAILURE! - Attendance Session for COURSE #".$course->id." failed to be inserted.\n";
			echo $logitem1."<br>";
		}//
		echo "---------------NEXT----------------------<br>";
		$logitems = $logitem.$logitem1;
...
fclose($fh);
	echo "<p><strong>Attendance Module addition completed.</strong></p>";//
	echo "<p>A log file named \"$logfile\" has been written to $directory</p>";//
	echo "<p>Return to <a href=\"/course/\">Course Categories</a></p>";
    exit;
} // if $adminator close
else
{
